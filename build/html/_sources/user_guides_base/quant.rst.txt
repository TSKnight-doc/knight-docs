...
-if或--infer-func
    ...
    - 可选 
    - 默认值：infer_auto
    - 说明：前向推理函数名称。注意: infer_auto为量化工具内置的自动根据输入shape生成随机数据开展量化的infer函数，具体说明可参见FAQ文档。
...
-lsj或--load-scale-json
    ...
    - 可选 
    - 默认值：无
    - 说明：存放量化scale信息的json文件路径，支持相对路径和绝对路径。注意：该参数与-gsj/-mc互斥。
...
-i或--iteration
    ...
    - 可选 
    - 默认值：1
    - 说明：量化时模型执行推理的次数。注意：需要选取合适的样本数量才能达到理想的量化模型精度。支持传入多值进行不同数据量化，多值输入时使用空格分割例如-i 1 10
	  注意：①此参数多值输入会分别按照每个值进行量化。
...
-bs或--batch-size
    ...
    - 说明：量化模型时加载量化数据的batchsize大小，量化过程实际用到的数据量是iteration* batch-size。
	  注意：①此参数仅在模型为动态图时可配置，且需要用户在infer函数里实现相应逻辑才能生效。②量化后模型不支持动态图。若原始浮点模型为动态batch，则需设置此参数为与量化时数据相同的batch-size值，否则可能导致量化异常。
...
-ib或--ir-batch
    ...
    - 说明：设置量化后模型的batch-size。
	  注意：①仅当原始浮点模型为动态batch时此参数方生效，若为固定输入模型此参数不生效。不支持多输入为动态batch且量化后对应输入的batch不一致的场景。
...
-gt或--generate-template
    ...
    - 可选 
    - 默认值：无
    - 说明：若设置，会生成模型对应的混合量化模板json配置文件，文件保存在{--save-dir}/目录下。注意：run-mode为quant时才生效
	
-mc或--mix-config
    - 参数类型: string
    - 可选 
    - 默认值：无
    - 说明：混合量化json文件路径，支持相对路径和绝对路径，需要确保配套硬件是支持混合量化。注意：该参数与-lsj互斥。
...
-qm或--quant-mode
    ...
    - 说明：计算激活值的scale系数采用的方法，支持模式：
		...
		* all：分别执行上述所有模式进行量化
		注意：①支持传入多值进行不同数据量化，多值输入时使用空格分割例如-qm kl mse②此参数多值输入会分别按照每个值进行量化。
...
-od或--output-dequant
    ...
    - 可选 
    - 默认值：无
    - 说明：是否增加反量化，若设置，会在所有输出层算子前增加反量化算子。注意：若某输出层的上方算子为ArgMax或者软件层，则对此输出层不生效不插入反量化算子。
...
-c2chw或--convert2chw
    ...
    - 说明：Tensorflow模型输入的format为4维NHWC，可通过指定该参数使转出的onnx模型从输入开始的format都为NCHW。
	  注意：
	  1. 适用场景：1) Tensorflow 4维模型 2)只对模型第一层算子为Conv、AveragePool、GlobalAveragePool、MaxPool、GlobalMaxPool、Resize等有效； 
...
.. code-block:: python
    ...
    model = torchvision.models.resnet18() # 模型定义，模型中所有算子须遵从《TS.Knight-转换算子规格表》之规格。
        if weight_path:# 模型加载
