...
|  一级       | 二级目录  |开源/封闭 | 说明                            |
+=============+===========+==========+=================================+
| /TS-Kni     | /tools    | 开源     | 常用小工具。/model_check:       |
|ghtSoftware  |           |          | 检查点2和检查点3结果验证。      |
|             |           |          | 详情参见                        |
...
|                     |          |      | :doc:`SDK使用指南<../user_guides_base/sdk>`          |
+---------------------+----------+------+------------------------------------------------------+
| /TX5                |/RNE      | 封闭 | TX5112x系列和TX5239x201系列芯片                      |
|112x_TX5239x201-Lib  |-SIM-Lib  |      | Knight RNE模拟库，详情参见                           |
|                     |          |      | :doc:`SDK使用指南<../user_guides_base/sdk>`          |
+                     +----------+------+------------------------------------------------------+
|                     |/RN       | 封闭 | TX5112x系列                                          |
...
+---------------------+----------+------+------------------------------------------------------+
| TS.K                |          | 封闭 | Knight压缩工具详情参见                               |
|night-MC_XXX.tar.gz  |          |      | :doc:`模型压缩使用指南<../user_guides_advanced/mc>`  |
+---------------------+----------+------+------------------------------------------------------+
...
在使用 `Knight compare` 工具的 `--show-hist` 直方图功能时，应参考以下方式启动docker。该功能详情请参见《TS.Knight-量化使用指南》。
...
Knight Finetune库使用详情参见 `QAT使用说明`_  。
...
当执行 `Knight build` 则连续执行量化 ``quant`` 和 编译 ``compile``两个步骤，此时若需要同时使用命令行，则需要增加quant或compile前缀，示例如下。
...
指定预处理参数input-configs，即可使用配置文件的方式对输入的数据集进行预处理，无需编写python代码即可完成量化操作，详情参见 `量化使用指南`_，使用示例如下：
...
Compare工具使用前，需要准备好待比较的数据。量化数据的准备需要使用量化工具的compare运行模式。具体参考 `量化使用指南`_。模拟器数据的准备需要使用模拟器的--debug参数，
具体参考 `编译仿真性能分析使用指南`_ 。
...
**步骤三: 编译debug模型** ( `--opt-ddr`参数必须为0, 否则, DDR优化会导致导出数据和量化数据对不齐)
...
各芯片支持的高效算子、通用算子请参见相应芯片的 `支持算子列表`_。用户自定义算子的添加步骤请参见  `编译仿真性能分析使用指南`_ 中的自定义算子章节。
