...
<img src="./6ec92ebd70ed7e02c3a014285728131fa4daac40.png"
alt="../media/overview_img1.png" />
</figure>
...
**Knight
RNE编译器(Knight-RNE-Compiler)**:编译量化模型，产生RNE执行的指令配置文件。

**Knight RNE模拟器(Knight-RNE-Simulator)**
:用于仿真神经网络在RNE上推理计算过程，输出计算层的结果。

**Knight
RNE性能分析器(Knight-RNE-Profiling)**:用于分析神经网络在芯片RNE上执行时间和存储开销，并给出分析报告。
...
**Knight
Demo**:提供计算机视觉，智能语音等领域的端到端的运行示例，演示Knight工具链的使用流程和具体用法。
...
<figure class="align-center">
<img src="./e95a229251fafda20287ec20d2d8705378dc4827.png"
alt="../media/overview_img2.png" />
</figure>
...
<figure class="align-center">
<img src="./9f972f95e1510671670468880a5f15fc0a63d26f.png"
alt="../media/overview_liucheng.png" />
</figure>
...
3. 用户使用Knight
   RNE模拟库对测试数据进行推理后，需保证其推理结果和KnightRNE模拟器推理结果一致；
4. 用户使用Knight
...
为便于用户快速进行检查点2，3的结果验证，提供model_check.py脚本，可参考
[model_check.py使用说明](#model_check.py使用说明)
::::
...
<figure class="align-center">
<img src="./6357cb4bb68d680c05c4497589908aca2defdd47.png"
alt="../media/overview_img33.png" />
</figure>

ReleaseDocuments目录中为产品文档，示例如下：

<figure class="align-center">
<img src="./63b9ae4b135acbae9564e87fe19260b64662a6fd.png"
alt="../media/overview_img4.png" />
</figure>

ReleaseDeliverables目录中为软件产品，示例如下：

<figure class="align-center">
<img src="./f0fa8a1c09529afe113775a04b7367926b53f4ee.png"
alt="../media/overview_img5.png" />
</figure>

ts.knight-XXX.tar.gz为Knight镜像压缩包，参见 [运行镜像](#运行镜像)
，运行镜像后进入Knight容器，容器内文件目录如下表所示。
...
| g | | | |
| htSoftware | | | /model_check:                  |
| | | | 检查点2和检查点3结果验证。 |
| | | | |
| | | | 详情参见FAQ |
| | | | [model_check.py使用 |
| | | | 说明](#model_check.py使用说明) |
| | | | 。 |
+------------+----------+---------+--------------------------------+
| /TS | / | 开源 | Knight |
| - | R | | demo相关的模型和数据，和代码 |
...
| | | | [SDK使用指南]()             |
+---------------------+----------+------+-----------------------------+
| /T | /RNE | 封闭 | TX5215x系列， TX5 |
| X5215x_TX5239x200\_ | -SIM-Lib | | 239x200系列，TX5239x220系列 |
| TX | | | 和TX5239x300系列芯片Knight |
| 5239x220_TX5239x300 | | | RNE模拟库， 详情参见 |
| -Lib | | | [SDK使用指南]()             |
+---------------------+----------+------+-----------------------------+
| | /RN | 封闭 | TX5215x系列， TX5 |
| | E-RT-Lib | | 239x200系列，TX5239x220系列 |
...
| | | | 详情参见 [QAT使用说明]()    |
+---------------------+----------+------+-----------------------------+
| TS.K | | 封闭 | Knight压缩工具 详情参见 |
| night-MC_XXX.tar.gz | | | [模型压缩使用指南]()        |
+---------------------+----------+------+-----------------------------+
...
3) 安装前面下载的安装包（参考[下载docker安装包](#_下载docker安装包)）
...
<figure class="align-center">
<img src="./574101294b349dd7c25a3e89cfd0d88355d608f1.png"
alt="../media/overview_docker_images.png" />
</figure>
...
| | |
| | 2）"docker容器目录"不能使用 "/TS-KnightDemo" 和 |
...
<figure class="align-center">
<img src="./988ec6fa7a7dbc95a596d853e0d161adb2f1c379.png"
alt="../media/overview_help.png" />
</figure>
...
-Lib以及TX5336x_TX5256x-Lib使用详情参见[SDK使用指南]() 。

Knight Finetune库使用详情参见[QAT使用指南]() 。

## 支持芯片

TS.Knight工具链支持清微芯片型号参见 [支持芯片](#支持芯片) 。
...
<figure class="align-center">
<img src="./917eb0d032ea887cf4f329bbf4ea6ea9bfd58449.png"
alt="../media/overview_cmd.png" />
</figure>
...
> [\\\\192.168.60.89\\home\$\\chenfan\\Desktop\\version截图.PNG]{.image
> .placeholder original-image-src="./media/image15.png"
> original-image-title="" width="6.85833in" height="1.75139in"}

- -h: 查看帮助信息，界面显示参见[2.4.3章节](#运行示例)。
- -ch/\--chip:
...
> [\\\\192.168.60.89\\home\$\\chenfan\\Desktop\\compile.PNG]{.image
> .placeholder original-image-src="./media/image16.png"
> original-image-title="" width="6.85833in" height="2.87292in"}
...
compile Knight RNE编译
器，详情参见《TS.Knight-编译仿真性能分析使用指南》

compare Knight精度比较
工具，详情参第[6章节](#knight-compare工具介绍)

run Knight RNE模拟
器，详情参见《TS.Knight-编译仿真性能分析使用指南》

profiling Knight RNE性能分析
器，详情参见《TS.Knight-编译仿真性能分析使用指南》
...
当执行Knight build则连续执行量化"quant"和
编译"compile"两个步骤，此时若需要同时使用命令行，则需要增加quant或compile前缀，示例如下。
...
[\\\\192.168.60.89\\home\$\\chenfan\\Desktop\\捕获1.PNG]{.image
.placeholder original-image-src="./media/image17.png"
original-image-title="" width="6.85833in" height="2.57083in"}
...
+=============+========+========+=====================================+
| -f或 | 必需 | 无 | 表示 |
| \ | | | 原始模型框架类型，可选范围{pytorch, |
| --framework | | | caffe, tf, onnx, paddle} |
+-------------+--------+--------+-------------------------------------+
...
| | | | d |
| \--quant-dir | | | ump目录的父目录的路径。参考《TS. |
| | | | Kn |
| | | | ight-量化使用指南》的\--run-mode |
| | | | compare参数说明。 |
| | | | |
| | | | 如果进行量化-浮点或者量化 |
| | | | - |
...
| | | | ，默认显示10条，当\--s |
| \ | | | how-level设置为0时，不显示数据。 |
| --show-number | | | |
+---------------+--------+------+----------------------------------+
| -si或 | 可选 | F | 显示对比数 |
| | | alse | 据时，是否显示索引，设置该参数时 |
| \--show-index | | | 表示显示索引值，不设置时不显示。 |
+---------------+--------+------+----------------------------------+
| \--save-dir | 可选 | 标准 | 指定比较结果的保存路径 |
| | | 输出 | 。如果没有指定，则输出到屏幕上。 |
+---------------+--------+------+----------------------------------+
| \--no-mre | 可选 | F | 不输出MRE指标 |
| | | alse | |
+---------------+--------+------+----------------------------------+
| \--no-cos | 可选 | F | 不输出余弦相似度指标 |
| | | alse | |
+---------------+--------+------+----------------------------------+
| \--rmse | 可选 | F | 输出均方根误差 |
| | | alse | |
+---------------+--------+------+----------------------------------+
| \--maxdiff | 可选 | F | | 单点最大误差(两个数值m1/m2)    |
| | | alse | | m1: 最大的相对误差 |
| | | | |
| | | | m2: 最大的绝对误差 |
+---------------+--------+------+----------------------------------+
| \--bias | 可选 | F | 有偏性(两个数值b1/b2)            |
| | | alse | |
| | | | b1：右边数\>左边数的百分比 |
| | | | |
| | | | b2：左边数\>右边数的百分比 |
...
| | | | |
| | | | m |
| | | | axdiff和bias有两个数值，先按照第 |
| | | | 一个数值降序排列，如果第一个数值 |
| | | | 相同，则按照第二个数值降序排列。 |
| | | | |
| | | | 缺省不排序。 |
+---------------+--------+------+----------------------------------+
| \--show-plot | 可选 | F | 显示指定精度指标的折线图。 |
| | | alse | |
| | | | 缺 |
| | | | 省不显示折线图，只显示统计列表。 |
+---------------+--------+------+----------------------------------+
...
[image6]{.image .placeholder original-image-src="./media/image18.png"
original-image-title="" width="6.85833in" height="4.02569in"}
...
[image7]{.image .placeholder original-image-src="./media/image19.png"
original-image-title="" width="6.85833in" height="3.66111in"}
...
| x_                                                                    |
| use_onnx/rne\*\*:\*\*/TS-KnightDemo/Output/resnet18_onnx_use_onnx/rne |
+=======================================================================+
| |
+-----------------------------------------------------------------------+

[image8]{.image .placeholder original-image-src="./media/image20.png"
original-image-title="" width="6.85833in" height="4.51042in"}
...
[image9]{.image .placeholder original-image-src="./media/image21.png"
original-image-title="" width="3.94934in" height="3.408in"}
...
[image10]{.image .placeholder original-image-src="./media/image22.png"
original-image-title="" width="6.85833in" height="3.05833in"}
...
[image11]{.image .placeholder original-image-src="./media/image23.png"
original-image-title="" width="6.85833in" height="3.72778in"}
...
[image12]{.image .placeholder original-image-src="./media/image24.png"
original-image-title="" width="6.85833in" height="3.82431in"}
...
[image13]{.image .placeholder original-image-src="./media/image25.png"
original-image-title="" width="3.19931in" height="2.53542in"}

同时会在屏幕上输出每个output index对应的输出名称。

[image14]{.image .placeholder original-image-src="./media/image26.png"
original-image-title="" width="2.47066in" height="4.40681in"}
...
[image15]{.image .placeholder original-image-src="./media/image27.png"
original-image-title="" width="2.80208in" height="2.43403in"}
...
[image16]{.image .placeholder original-image-src="./media/image28.png"
original-image-title="" width="1.9875in" height="0.72431in"}
...
[image17]{.image .placeholder original-image-src="./media/image29.png"
original-image-title="" width="6.85833in" height="3.55139in"}
...
[image18]{.image .placeholder original-image-src="./media/image30.png"
original-image-title="" width="6.85833in" height="2.6625in"}
...
#### 显示文件内容
  ...
  -----------------------------------------------------------------------
show_sim_result \--sim-data /TS-KnightDem
o/Output/resnet18_onnx_use_onnx/rne/dump/result-maxpool-maxpool_p.txt
  -----------------------------------------------------------------------

  -----------------------------------------------------------------------

[image19]{.image .placeholder original-image-src="./media/image31.png"
original-image-title="" width="6.85833in" height="1.30347in"}
...
[image20]{.image .placeholder original-image-src="./media/image32.png"
original-image-title="" width="6.85833in" height="0.41111in"}

+-----------------------------------+-----------------------------------+
| show_sim_result \\ | |
| | |
| \--sim-data /TS-KnightDemo/O | |
...
[image21]{.image .placeholder original-image-src="./media/image33.png"
original-image-title="" width="6.85833in" height="0.69653in"}
...
[image22]{.image .placeholder original-image-src="./media/image34.png"
original-image-title="" width="6.85833in" height="0.80486in"}
...
[image23]{.image .placeholder original-image-src="./media/image35.png"
original-image-title="" width="6.85833in" height="2.89722in"}
...
用户执行完量化命令和编译命令后，可使用该脚本进行检查点2，检查点3（参见[章节1.3.2](#模型资源生成开发流程)）结果验证，仅支持单路输入模型，当模型具有多路输出时仅对比最后一路结果。
