...
.. note::
    注意：Knight压缩工具、Knight量化工具、Knight RNE编译器、Knight RNE模拟器和Knight RNE性能分析器所有芯片均支持，Knight RCE仅在部分芯片支持，当前仅TX5368x系列，TX5339x系列和TX5335x系列芯片支持。
...
ts.knight-XXX.tar.gz为Knight镜像压缩包，参见 `运行镜像`_ ，运行镜像后进入Knight容器，容器内文件目录如下表所示。
...
在使用Knight compare工具的--show-hist直方图功能时，应参考以下方式启动docker。该功能详情请参见《TS.Knight-量化使用指南》。
...
-Lib以及TX5336x_TX5256x-Lib使用详情参见`SDK使用指南`_ 。

Knight Finetune库使用详情参见`QAT使用指南`_ 。
...
下面是包含”quant”字段的完整json配置文件参考，详细信息请参考`量化使用指南`_。
